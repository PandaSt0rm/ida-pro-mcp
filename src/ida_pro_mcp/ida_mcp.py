...
class MCP(idaapi.plugin_t):
    ...
    def init(self):
        ...
        # Auto-start once an IDB is available. If a database is already open
        # during plugin init, start immediately; otherwise wait for the next
        # open-database notification and unregister the callback after it fires.
        import ida_idaapi
        ...
        if ida_nalt and hasattr(ida_nalt, "get_root_filename"):
            ...
            get_root_filename = idaapi.get_root_filename
        ...
        def autostart(nw_code, is_old_db=False):  # noqa: ARG001
            ida_idaapi.notify_when(
                ida_idaapi.NW_OPENIDB | ida_idaapi.NW_REMOVE, autostart
            )
            start_once()

        root_loaded = False
        try:
            if get_root_filename and get_root_filename():
                root_loaded = True
        except Exception:
            root_loaded = False
        ...
        if root_loaded:
            ...
            ida_idaapi.notify_when(ida_idaapi.NW_OPENIDB, autostart)
